...
const uiLabels: LocalizedLabels = {
  ...
  'Show all the information we have about the update.': '顯示我們掌握的所有有關此更新的信息。',
...
const steps: LocalizedSteps = {
  ...
  'DifferFactory': {
    action: '如果調用 `DifferFactory.create(...)`，移除 `ChangeDetectorRef` 參數。'
  },
  ...
  'node 8': {
    action: '請確保使用的是 <a href="http://www.hostingadvice.com/how-to/update-node-js-latest-version/" target="_blank">Node 8 或更高版本</a>'
  },
  'Update to CLI v6': {
    action: '在本地更新 Angular CLI，並將配置更新到<a href="https://github.com/angular/angular-cli/wiki/angular-workspace" target="_blank">新的 angular.json 格式</a>通過運行以下命令:<br/>`${packageManagerInstall} @angular/cli@6`<br/>`ng update @angular/cli@6`<br/>'
  },
  'cli v6 scripts': {
  ...
  'create commits': {
    action: '您可以選擇將 `--create-commits` (或 `-C`)標誌傳遞給 <a href="https://angular.io/cli/update" target="_blank">ng update</a> 命令，以針對每次遷移創建 git commit。'
  },
  'ng update v9': {
    action: '在終端中運行 `ng update @angular/cli@9 @angular/core@8` 更新 CLI 和核心框架到 v9。'
  },
  'typescript 3.8': {
    action: '您的項目現已更新為 TypeScript 3.8，在 <a href="https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-7.html" target="_blank">TypeScript 3.7 公告</a> 或 <a href="https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-8.html" target="_blank">TypeScript 3.8 公告</a>閱讀有關新編譯器檢查和錯誤的詳細信息，這些檢查和錯誤可能需要您修復代碼中的問題。'
  },
  ...
  'ivy update': {
    action: '在更新到版本 9 的過程中，需要通過代碼遷移對項目進行轉換，以便從代碼庫中刪除所有不兼容或不建議使用的 API 調用。您現在可以查看這些更改，並查閱<a href="https://v9.angular.io/guide/updating-to-version-9" target="_blank">更新至版本 9 </a>以瞭解有關更改的更多信息。'
  },
  'stylesUpdate': {
  ...
  'v11 browser support': {
    action: '不再支持 IE9，IE10 和 IE mobile。這是在 <a href="http://blog.angular.io/version-10-of-angular-now-available-78960babd41#c357" target="_blank">Angular 版本 10 更新</a> 中宣佈的。'
  },
  'webpack5 optin': {
  ...
  'v11 CollectionChangeRecord': {
    action: '在更改檢測中，`CollectionChangeRecord` 已移除，應改用 `IterableChangeRecord`。'
  },
  'v11 forms async validators': {
    action: '如果在初始化時對 `FormControl`、`FormGroup` 或 `FormArray` 類實例上定義的異步驗證器使用 Angular Forms，則在異步驗證器完成後，以前不會發出狀態更改事件。這已被更改，因此 status 事件被髮送到 `statusChanges` observable 中。如果代碼依賴於舊的行為，則可以忽略此附加的狀態更改事件。'
  }
};
